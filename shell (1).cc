...
{ std::cout << "ToDo: Implementeer hier new_file()" << std::endl;
  ...
  int fd = syscall(SYS_creat, filename.c_str(), 0777);
  
  ssize_t a = syscall(SYS_write,fd, filetext.c_str(), filetext.size());
...
{ 
  if(fork()== 0){
      ...
      syscall(SYS_execve, "/bin/ls", arguments);
  }
  else{
      int eindstatus;
      wait(&eindstatus);
...
{ std::cout << "wat zoek je" << std::endl;
  ...
  if(id == 0){
      ...
      syscall(SYS_close, pipezoek[0]);
      syscall(SYS_dup2, pipezoek[1], STDOUT_FILENO);
      syscall(SYS_execve, "/usr/bin/find",arguments);
  }
  id = fork();
  if(id == 0){
      char *parameter[]= {(char*)"grep", (char*)zoeken.c_str(), (char*)0};
      syscall(SYS_close, pipezoek[1]);
      syscall(SYS_dup2, pipezoek[0], STDIN_FILENO);
      syscall(SYS_execve, "/usr/bin/grep",parameter);
  }
  syscall (SYS_close, pipezoek[0]);
  syscall(SYS_close, pipezoek[1]);
  syscall(SYS_wait4, -1, &status, 0);
  syscall(SYS_wait4,-1, &status, 0);
   }
...
{ std::cout << "ToDo: Implementeer hier seek()" << std::endl;
  ...
  int seek_make = syscall(SYS_creat,"seek.txt", 0777);
  int loop_make = syscall(SYS_creat, loop.c_str(), 0777);
  syscall(SYS_write, loop_make, "x", 1);
  std::string message = "\0";
  for(int i = 0; i < 50000; i++){

syscall(SYS_write, loop_make, "\0", 1);

}
syscall(SYS_write, loop_make, "x", 1);
syscall(SYS_write, seek_make, "x", 1);
syscall(SYS_lseek, seek_make, 50000, SEEK_END);
syscall(SYS_write, seek_make, "x", 1);
